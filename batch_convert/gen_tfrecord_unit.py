...
class Write_Tfrecord():
    ...
    def image_and_gtbox_label(self, image_name_with_format, img_dir, gt_dir):
        ...
        """
        image_name_prefix, _ = os.path.splitext(image_name_with_format)
        image_data = tf.gfile.GFile(os.path.join(img_dir, image_name_with_format), 'rb').read()
        image_mat = scipy.misc.imread(io.BytesIO(image_data))
        ...
        gtbox_labels = []
        txt_path = os.path.join(gt_dir, "{}.txt".format(image_name_prefix))
        ...
        return image_name_prefix, image_data, gtbox_labels
    ...
    def convert_to_tfrecord(self):
        ...
        img_fn_list = img_fn_list[self.sample_idx_range[0]: self.sample_idx_range[1]]
        tfrecord_path = os.path.join(os.path.abspath(self.tfrecord_dir),
                                     "{}_{}_{}_{}.tfrecord".format(config.Dataset_name,
                                                                   self.subset,
                                                                   str(self.sample_idx_range[0]),
                                                                   str(self.sample_idx_range[1])
                                                                   ))
        ...
        with tf.python_io.TFRecordWriter(tfrecord_path) as tfrecord_writer:
            pbar = tqdm(total=len(img_fn_list), desc='thread_%d'%self.thread_id, ncols=80)
            for image_name_with_format in img_fn_list:
